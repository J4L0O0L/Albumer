...
public struct ImageViewer: View {
    ...
    public init(imageURL: String, viewerShown: Binding<Bool>, aspectRatio: Binding<CGFloat>? = nil, disableCache: Bool? = nil, caption: Text? = nil, closeButtonTopRight: Bool? = false) {
        ...
        loader = ImageLoader(urlString:imageURL)
    }
                    ...
                    VStack {
                        ZStack {
                            ...
                            if(url != nil){
                                ...
                                ).aspectRatio(self.aspectRatio?.wrappedValue, contentMode: .fit).offset(x: self.dragOffset.width, y: self.dragOffset.height).rotationEffect(.init(degrees: Double(self.dragOffset.width / 30)))
                                    .pinchToZoom()
                                    .gesture(DragGesture()
                                                .onChanged { value in
                                ...
                                //                                    .onReceive(loader.didChange) { data in
...
class ImageViewLoader: ObservableObject {
    ...
    func load() {
        cancellable = URLSession.shared.dataTaskPublisher(for: getURLRequest(url: self.url))
